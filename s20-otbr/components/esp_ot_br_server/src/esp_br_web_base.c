...
cJSON *otbr_properties_struct_convert2_json(openthread_properties_t *properties)
{
    ...
    sprintf(format, "%u", properties->information.thread_version);
    cJSON_AddStringToObject(root, "OpenThread:ThreadVersion", format);
                                                       ...
                                                       thread_network_formation_param_t *param)
{
    ...
    if (temp && temp->type != cJSON_NULL)
        param->default_route = cJSON_IsTrue(temp);
    else
